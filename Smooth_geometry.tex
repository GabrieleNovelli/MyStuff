...
\begin{document}
	...
	\begin{Obs}
		...
		$$\phi_{\vec{e}}:{GL(n,\mathbb{R})\over Stab(\vec{e})}\rightarrow O(\vec{e})=Fr(V);\hbox{ such that } \phi_{\vec{e}}([A])=\vec{e}A$$
		The idea now is to put a manifold structure on $Fr(V)$ such that the above map becomes a diffeomorphism.
	\end{Obs}
	\end{document}
