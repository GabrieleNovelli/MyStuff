...
\begin{document}
	...
	In this chapter we introduce the notion of distribution. This is an extremely wide topic, which evolves mainly around the infamous Frobenius theorem. The idea is simple: when does a distributuion of tangent vectors define a smooth manifold?
		...
		\begin{Def}
			We call \textit{distribution} $D$ on a manifold $M$ a smooth assignment $p\mapsto D_p\subset T_pM$ such that: for any $p\in M$ one can find $U$ open containing $p$ such that for all $q\in U$ there is a collection of vector fields $\{X_i(q)\}$ that span $D_q$.
		\end{Def}
		...
		\begin{Def}
 		...
 		\end{Def}
 		\begin{Obs}
 			Clearly, in any flat chart, we have some immersed submanifolds, defined by keeping the corresponding coordinates fixed.
 		\end{Obs}
 		\begin{Def}
 			Let $D\subset TM$ be a completely integrable constant rank distribution. Then the immersed submanifold defined by it are called \textit{integral manifolds}.
 		\end{Def}
 		...
 		\end{proof}
 		\begin{Def}
 			Let $M$ be a smooth manifold. We call \textit{$k$-dimensional foliation} $\mathcal{F}$ a family of $k$-dimensional immersed submanifolds of $M$ such that:
 			\begin{itemize}
 		...
 		\end{Cor}
 		\section{A generalization of Frobenius theorem}
 		...
 		\begin{Def}
 			A distribution $D$ is called \textit{homogeneous} if it is invariant under the flow of its vector field, i.e.:
 		...
 		\begin{Ex}
 			...
 			\begin{itemize}
 				...
 				$$(Fl_t^{\partial_x})_*=\begin{pmatrix}
 					...
 					0 & 0 & 1
 				\end{pmatrix}$$
 				This clearly preserves the distribution.
 			...
 			\end{itemize}
 			It is worth noticing that the distribution in the first example is involutive, while the second isn't.
 		\end{Ex}
 		We now re-define the notion of adapted chart, dropping the costant rank requirement.
 		\begin{Def}
 		...
 		\begin{proof}
 			...
 			\begin{itemize}
 				...
 				\item[$(ii)\implies (iv)$]
 				Suppose the distribution is involutive, by REFERENZA we can find a local base of commuting vector fields around any point. From REFERENZA, since we have commuting fields, we can use them to make some coordinates:
 				$$\{\partial_1,...,\partial_r,\partial_{r+1},...,\partial_n\}$$
 				Since the distribution has constant rank along its integral curves, moving along the directions spanned from the first $r$ vectors produces no change in rank. This is clearly an adapted chart.
 				\item[$(iv)\implies (i)$]
	...
	\end{Def}
	\begin{Def}
		We call \textit{geodesic} a curve which is parallel to itself:
		$$\gamma(t)\hbox{ such that }\nabla_{\dot{\gamma}}\dot{\gamma}(t)=0$$
	\end{Def}
	Note that the notion of geodesic is completely independent from the choice of the bundle we are woirking with. We will however be more interested in metric-related geodesics.
