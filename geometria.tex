...
\begin{document}
	...
	It is in general useful to think of the Clifford algebra as a quite general deformation of the exterior algebra of a vector space. This will be clear from the definition. It is, intuitively, for this reason that those two structures can be shown to be canonically isomorphic.
	\\\\
	We consider vector spaces endowed with qwuadratic forms $(V,Q)$. We will refer to such spaces as \textit{quadratic vector spaces} for simplicity.
	\begin{Def}
	...
	\end{Obs}
	It is immediately clear tha tthe definition of the CLifford algebra is litterally a deformation of the exterior algebra.
	\begin{Obs}
		Clearly, if the form is trivial i.e. $Q(v)=0$ always, $Cl(V,Q)=\bigwedge V$.
	\end{Obs}
	\begin{Obs}
		The Cliffor algebra has a product:
		$$\omega\cdot \tau=[\omega\otimes \tau]=\omega\otimes \tau+I_Q$$
		This product is clearly bilinear and associative. Note also that:
		$$\omega^2=[\omega^2]=Q(v)$$
		We also have that:
		$$(\omega+\tau)(\tau+\omega)=\omega^2+\tau^2+\omega\cdot\tau+\tau\cdot\omega=Q(\omega)+Q(\tau)+\omega\tau+\tau\omega=$$
		$$=2\braket{\omega,\tau}-Q(\omega+\tau)$$
		Thus:
		$$\omega\tau+\tau\omega=2\braket{\omega,\tau}$$
		We will re-write this in the anti-commutator fashion:
		$$\{\omega,\tau\}=2\braket{\omega,\tau}$$
	\end{Obs}
	\begin{Def}
		Let $(V,Q)$ be a quadratic space. A basis $\{e_i\}$ of $V$ is said to be \textit{orthonormal} for $Q$ if:
		$$Q(e_i+e_j)=Q(e_I)+Q(e_j)+\delta_{ij}$$
	\end{Def}
	Substantially, $\braket{e_I,e_J}=\delta_{ij}$.
	\begin{Def}
		Let $v\in V$ and $\omega\in \bigwedge^k V$. We define the \textit{contraction} $v\rfloor \omega$ as the unique $k-1$ form so that:
		\begin{itemize}
			\item if $\omega\in \bigwedge^1 V$ then $v\rfloor\omega=Q(v,\omega)$
			\item the contraction is an antiderivation:
			$$v\rfloor(\omega\wedge \tau)=v\rfloor\omega\wedge \tau+(-)^{deg(\omega)}\omega\wedge v\rfloor\tau$$
		\end{itemize}
	\end{Def}
	\begin{Prop}[\textbf{Universal property of the Clifford algebra}]
		Let $(V,Q)$ be a quadratic vecctor space on the field $\mathbb{K}$. Let $\mathbb{A}$ be an associative algebra on $\mathbb{K}$. Then, for any linear map $f:V\rightarrow A$ such that:
		$$\{f(v),f(w)\}=2\braket{v,w}1_A$$
		there is a unique algebra homomorphism $\phi:Cl(V,Q)\rightarrow \mathbb{A}$ such that the following diagram commutes:
		$$
		\begin{tikzcd}
			V\arrow{rr}{Q}\arrow{ddrr}{f}&& Cl(V,Q)\arrow{dd}{\phi}\\\\
			&&\mathbb{A}
		\end{tikzcd}$$
	\end{Prop}
	\begin{proof}
		The idea is to use the tensor product universal property. In particular, consider:
		$$g:V^k\rightarrow \mathbb{A} \hbox{ like }g(v_1,...,v_k)=f(v_1)\cdot...\cdot f(v_k)$$
		This is by construction multilinear. Thus there is a unique map from $\otimes^kV$ to $\mathbb{A}$ like:
		$$\tilde{g}:\otimes^k V\rightarrow\mathbb{A}\hbox{ like }\tilde{g}(v_1\otimes...\otimes v_k)=g(v_1,...,v_k)$$ 
		Clearly, by hypothesis:
		$$\tilde{g}(v\otimes v-Q(v))=f(v)^2-Q(v)=0$$
		This means that $Ker(\tilde{g})$ contains $I_Q$ and so this map descends to another map:
		$$\hat{g}:Cl^k(V,Q)\rightarrow \mathbb{A}$$
		This holds true for every degree $k$ and so:
		$$\phi:Cl(V,Q)\rightarrow \mathbb{A}\hbox{ is like }\phi(v_1\cdot...\cdot v_k)=f(v_1)\cdot...\cdot f(v_k)$$
		Clearly:
		$$\phi\circ Q(v)=\phi(v^2)=f(v)^2=\braket{v}^21_A$$
	\end{proof}
	The universal property is unfortunately for us fundamental, as from it wew can recover some very useful isomorphims:
	\begin{Theo}
		There is a canonical isomorphism:
		$$Cl(V,Q)\simeq \bigwedge V$$
	\end{Theo}
	\begin{proof}
		Consider the map:
		$$\delta: V\rightarrow End\big(\bigwedge V\big)\hbox{ like }\delta(v)(\alpha)=v\wedge\alpha-v\rfloor\alpha$$
		This map is clearly linear and 
		$$\{\delta(v),\delta(w)\}(\alpha)=(v\wedge w+w\wedge v)\wedge \alpha-v\wedge (w\rfloor\alpha)+v\rfloor (w\rfloor\alpha)-w\wedge (v\rfloor\alpha)+w\rfloor (v\rfloor\alpha)-v\rfloor(w\wedge \alpha)-w\rfloor(v\wedge \alpha)$$
		Since the contraction acts as an antiderivation and $\omega,v$ are of degree 1:
		$$-v\wedge (w\rfloor \alpha)=w\rfloor(v\wedge \alpha)-\braket{v,w}\wedge\alpha$$
		This means:
		$$\{\delta(v),\delta(w)\}(\alpha)=(v\wedge w+w\wedge v)\wedge \alpha-2\braket{v,w}\wedge\alpha$$
		Clearly, $v\wedge w=v\cdot w-Q(w)-Q(v)$, so that:
		$$\{\delta(v),\delta(w)\}(\alpha)=2\braket{v,w}$$
		so, from the universal property REFERENZA, it descends to a well defined unique linear mapping:
		$$f:Cl(V,Q)\rightarrow End\big(\bigwedge V\big)\hbox{ like }f([x])=\delta(x)$$
		In particular, consider:
		$$g:Cl(V,Q)\rightarrow \bigwedge V$$
		defined like $g([x])=f([x])(1)=\delta(x)1$. This amounts to:
		$$g(v)=$$
		
	\end{proof}
	\begin{Cor}
		From the previosu theorem we get that $dim(Cl(V,Q))=2^n$ and that given an orthonormal basis $\{e_i\}$ we have a basis for $Cl(V;Q)$ like:
		$$e_{i_1}\cdot e_{i_2}\cdot...\cdot e_{i_k}$$
	\end{Cor}
	\begin{Def}
		Let $(V,Q)$ be a quadratic vector space, let $\{e_i\}$ be an orthonormal base and $\rho:Cl(V;Q)\rightarrow End(\mathbb{K}^r)$  a representation.
		We define the \textit{gamma matrices} of a Clifford algebra $Cl(V,Q)$ the set of elements:
		$$\Gamma_a=-i\rho\circ e_a$$
	\end{Def}
	Clearly, we have the following relations:
	$$[\Gamma_a,\Gamma_b]=2\braket{a,b}1_r\hbox{ and }\{\Gamma_a,\Gamma_b\}=-2\braket{a,b}1_r$$
	\section{The Chirality}
	In this section we look at the Chirality operator. In general, this will have a complex nature.\\\\
	From now on, real Clifford algebras will be indexed as $Cl(V,Q)$, while complex ones $\mathbb{C}l(V,Q)$.
	\begin{Def}
		Let $Cl(V,Q)$ be a Clifford algebra. We define a \textit{chirality operator} as:
		$$\omega=\lambda e_i...\cdot e_n$$
		where $\{e_i\}$ is an orthonormal basis.
	\end{Def}
	\begin{Obs}
		Clearly, since there are only two possible orientations for a vector space, this element is defined up to a sign, for every orthonormal base in the same way.
	\end{Obs}
	\begin{Prop}
		The following holds:
		\begin{itemize}
			\item $\{\omega,e_a\}=0$;
			\item $\{\omega,e_a\cdot e_b\}=0$;
			\item $\omega^2=\lambda^2$
		\end{itemize}
	\end{Prop}
	\begin{proof}
		This clearly follows from the definition of orthonormal basis:
		$$e_a\cdot \omega=\lambda e_a \cdot e_1\cdot...\cdot e_n=(-)^{a-1}\lambda e_1...e_a\cdot e_a...e_n=-e_a\cdot \omega$$
		The other requests follows from having $2n$ elements.
	\end{proof}
	\begin{Def}
		Suppose $(V,Q)$ is a quadratic vector space with $dim(V)=2k$ and suppose $Q$ has signature $(s,t)$; $s+t=2k$. We define the \textit{canonical chirality element} as:
		$$\omega=-i^{k+t} e_1...e_n$$
		In the case of a real clifford algebra we have $\omega\in Cl(V,Q)\otimes \mathbb{C}$. For a complex Clifford algebra instead $\omega\in\mathbb{C}l(V,Q)$
	\end{Def}
	\begin{Prop}
		There is an isomorphism:
		$$Cl(V,Q)\otimes \mathbb{C}\simeq\mathbb{C}l(V,Q)$$
		This isomorphism preserves the chirality element.
	\end{Prop}
	\begin{proof}
		Consider the map:
		$$f:C^n\simeq \mathbb{R}^n\otimes \mathbb{C}\rightarrow Cl(n)\otimes \mathbb{C}\hbox{ like }v\otimes z\mapsto [v]\otimes z$$
		Taking the square:
		$$f(v)^2=[v^2]\otimes z=Q(v)z$$
		This imples:
		$$\{f(v),f(w)\}=2\braket{v,w}$$
		From the fundamental property there is a well defined map:
		$$\delta: \mathbb{C}l(n)\rightarrow Cl(n)\otimes \mathbb{C}\hbox{ like }\delta([v])=f(v)$$
		This is an isomorphism since it is surjective clearly, and the dimensions coincide.
	\end{proof}
	...
	In this chapter we introduce the notion of distribution. This is an extremely wide topic, which evolves mainly around the infamous Frobenius theorem. The idea is simple: when does a distributuion of tangent vectors define a smooth manifold?
 		...
 		\begin{Ex}
 			...
 			\begin{itemize}
 				...
 				\item Consider $\{x\partial_x,\partial_y\}$ defined in $\mathbb{R}^3$. The integral curves are:
 				$$\gamma_1(t)=(e^tx,y,z) \hbox{   and   }y(t)=(x,y+t,z)$$
 				It seems like $x\partial_x$ might not have constant rank along its own integral curves. However, it takes an infinite time, for $x\neq 0$, to get to $x=0$ along the integral cuve. Thus, this distribution is homogeneous.
 				\item Consider the distribution:
 			...
 			\end{itemize}
 			It is worth noticing that the distributions in the first, second and third examples are involutive, while the other aren't.
 		\end{Ex}
 		\begin{Obs}
 		...
 		\begin{proof}
 			...
 			\begin{itemize}
 				...
 				Given the initial condition $\gamma(0)\in D_p$, since the flux of the fields spans the distribution, $\gamma_i$ is inside the distribution at any $t$. To see this, we can write the solution as:
 				$$\gamma(t)=\bigg(exp\mathcal{T}\int_0^t[\lambda^j_i(\tau)]d\tau\bigg) \gamma(0)$$
 				This is known as the \textit{Peano-Baker serie} REFERENZA and it is clear that if the solution at $t=0$ is inside $D_p$, then at any $t$ the solution will still be inside it.
 				\item[$(iii)\implies (iv)$]
 				...
 				Clearly, $\phi(0,...,0)$ is a vector space isomorphism, since we have selected a local frame around $p$. This means that we can find a small neighbourhood of $(0,...,0)$ where $\phi$ is aa diffeomorphism. We thus define the chart $(U,\phi^{-1})$ in this little set. It remains to show that this is the adapted chart.
 				By construction:
 				$$\partial_i=\phi^{-1}_*\big({\partial\over \partial{t^i}}\big)={d\over dt}\phi(t_1,...,t_i+t,...,t_n)=$$
 				$$=(Fl_{t_1}^{X_1})_*\circ...\circ (Fl_{t_{i-1}}^{X_{i-1}})_*X_i\bigg|_{Fl_{t_i}^{X_i}\circ...\circ Fl_{t_n}^{X_n}}$$
 				By homogeneity, this is the push-forward along an integral curve of the distribution fieldsand since $X_i\in D$ for $i<r+1$ in all of the set $U$, we have that also $\partial_i$ is, for $i<r+1$.\\
 				Fixing a slice $(t_1,...,t_r,\vec{t})$ we can clearly move along the integral curves of $D$. Since the distribution is homogeneous, and the local flows are diffeomorphism (at fixed time), we have that linear independence is preserved along integral curves. This proves that the rank of each slice is constant.
 				\item[$(iv)\implies (i)$]
	...
	\end{proof}
	We must now lose a little time to focus on the properties of local trivializations. Those will come in handy later on:
	\begin{Prop}
		The following holds:
		\begin{itemize}
			\item $g_{ij}g_{jk}=g_{ik}$;
			\item $g_{ii}=e$;
			\item $g_{ij}=g_{ji}^{-1}$.
		\end{itemize}
	\end{Prop}
	\begin{proof}
		Those are all obvious from the fact that trivializations are diffeomorphisms. 
	\end{proof}
	One of the main differences between principal and vector bundles are the global properties. In particular, recall that, as seen in REFERENZA, it is not sufficient for a vector bundle to have a global section to be trivialized. This is instead a sufficient condition for principal bundles. The reason will be clear in a while.
	\begin{Prop}
		Given a local gauge of a principal $G$-bundle, it uniquely defines a trivialization.
	\end{Prop}
	\begin{proof}
		Consider $s:U\rightarrow P$ a local gauge. We can define:
		$$\phi^{-1}(x,g)=s(x)\cdot g$$
		This is clearly smooth and equivariant. The bijectivity can be shown in the following way. Take $p\in\pi^{-1}(U)$, then there will be $g\in G$ such that $s(x)\cdot g=p$. This follows from the transitivity of the action. 
	\end{proof}
	\begin{Cor}
		If a principal bundle has a global section then it is trivial.
	\end{Cor}
	\begin{Cor}
		Any two overlapping gauges differ by a transition function.
	\end{Cor}
	\begin{proof}
		Consider two overlapping gauges $s_{i,j}:U\rightarrow P$. Then, as previously seen, those define two trivializations:
		$$\phi_{i,j}^{-1}(x,h)=s_{i,j}(x)\cdot h$$
		Moreover:
		$$\phi_i\circ \phi_j^{-1}(x,h)=(x,g_{ij}(x)h)=\phi_i(s_j(x)\cdot h)$$
		Applying $\phi_i$ to both sides:
		$$s_j(x)\cdot h=\phi_i^{-1}(x,g_{ij}h)=s_i(x)\cdot g_{ji}(x)\cdot h$$
		This clearly proves:
		$$s_j(x)=s_i(x)\cdot g_{ji}(x)$$
	\end{proof}
	\begin{Ex}
		We casn make some easy examples to grasp the significance of the information above:
		\begin{itemize}
			\item Consider the trivial bundle $\mathbb{R}^2\times U(1)\rightarrow \mathbb{R}^2$. This bundle is trivial and so there is a global trivialization: the identity map. Consider the following section:
			$$s:\mathbb{R}^2\rightarrow \mathbb{R}^2\times U(1)\hbox{ like }s(x,y)=((x,y),\lambda(x,y))$$
			where $\lambda(x,y)$ is a smooth map: $\lambda:\mathbb{R}^2\rightarrow U(1)$. This defines the following trivialization:
			$$\phi^{-1}((x,y),\gamma)=s(x,y)\cdot \gamma=((x,y),\lambda(x,y)\gamma)$$
			\item Consider the Hopf bundle:
			$$
			\begin{tikzcd}
				U(1)\arrow{rr}&&P\arrow{dd}{\pi}\\\\
				&&M
			\end{tikzcd}$$
			This is a non-trovial bundle and so it has no global section. However, we can construct one locally. Consider the set:
			$$U_1=S^2\setminus(0,1)$$
			and the section:
			$$s:U_1\rightarrow S^3 \hbox{ like }s(z,x)=\bigg(\sqrt{1+x\over 2},{\bar{z}\over \sqrt{2(1+x)}}\bigg)$$
			This is well defined. To see this observe first of all that, given that $x^2+|z|^2=1$, we automatically have:
			$${1+x\over 2}+{|z|^2\over 2(1+x)}={(1+x)^2+|z^2|\over 2(1+x)}={2+2x\over 2(1+x)}=1$$
			Moreover:
			$$\pi\circ s(z,x)=\mathbb{I}$$
			Furthermore, just like in example REFERENZA, we have that ${\bar{z}\over \sqrt{2(1+x)}}$ is never 0 since if it was it would imply $x=1$. The trivialization defined by this local section is the following:
			$$\phi^{-1}((z,x),\lambda)=\bigg(\sqrt{1+x\over 2},{\bar{z}\over \sqrt{2(1+x)}}\bigg)\cdot \lambda$$
		\end{itemize}
	\end{Ex}
	\section{An important example: the Frame Bundle}
	In this section we will go through the construction and the study of one of the mostly know principal bundles: the Frame Bundle. The idea is simple: any smooth manifold has a canonical principal bundle associated to it: the one composed of all possible frames. This is a $GL(n,\mathbb{R})$ bundle in general.
	\chapter{Ehresmann geometries}
	In this chapter we will give the general definition of an Ehresmann connection and look at some of its properties. The Ehresmann connection is one of the most important tools in particle physics, as it represents gauge fields (photons, gluons ecc). The idea behind the definition of an Ehresmann connection is extremely simple: to identify an horizontal distribution in the principal bundle. Goemetrically, this translates into a form, which vanishes in such directions.
	\section{Fundamental vector fields}
	In this section we define fundamental vector fields. The notion of fundamntal vector fields is crucial to define a connection. In particular, we ought to give a deepeer meaning to vertical vectors.
	\begin{Def}
		Let $P$ be a principal $G$-bundle over $M$ and let $\mathfrak{g}$ be the Lie algebra of $G$. If $X\in\mathfrak{g}$, we define the \textit{fundamental vector field of $X$ starting at $p\in P$} as:
		$$\bar{X}={d\over dt}\bigg|_0 p\cdot e^{tX}\in T_pP$$
	\end{Def}
	Fundamental vector fields have some obvious properties.
	\begin{Prop}
		Fundamental vector fields are smooth.
	\end{Prop}
	\begin{proof}
		This is obvious since they are the starting vectors of the curve:
		$$c_p(t)=p\cdot e^{tX}$$
		and differentiating:
		$$c_p'(t)=\bar{X}_{c_p(t)}$$
	\end{proof}
	There is a nicer, more useful way, of describing fundamental vector fields. Define the map:
	$$j_p:G\rightarrow P \hbox{ like }j_p(g)=p\cdot g=\mu(p,g)$$
	This is clearly smooth.
	\begin{Obs}
		Differentiating the above map we get our fundamental vector field:
		$$(j_p)_*(X)={d\over dt}\bigg|_0 j_p(e^tX)={d\over dt}\bigg|_0 p\cdot (e^tX)=\bar{X}$$
		Isn't this way nicer?
	\end{Obs}
	\begin{Prop}
		The following property holds:
		$$(r_g)_*\bar{X}=\overline{Ad_{g^{-1}}(X)}$$
	\end{Prop}
	\begin{proof}
		This is a straightforward calculation:
		$$r_g\circ j_p(h)=p\cdot hg=p\cdot gg^{-1}hg=j_{p\cdot g}\circ c_g^{-1}(h)$$
		This immediately implies:
		$$(r_g\circ j_p)_*=(r_g)_*(j_p)_*=(j_{p\cdot g})_*(c_{g^{-1}})_*$$
		Since the differential of the conjugation is the adjoint, we get the result.
	\end{proof}
	We are almost done with fundamental vector fields. We now give an incredibly meaningful proposition:
	\begin{Prop}
		$\bar{X}$ vanishes if and only if $X$ is in the Lie algebra of $Stab(p)$.
	\end{Prop}
	\begin{proof}
		This is evident from the definitioon:
		$$\bar{X}={d\over dt}\bigg|_0 p\cdot (e^tX)$$
		Now, if $\bar{X}$ is 0 it means that we are differetniating something constant i.e. $e^{tX}\in Stab(p)$.
	\end{proof}
	\begin{Cor}
		The kernel of the differential of $j_p$ at the identity is the Lie algebra of the stabilizer of $p\in P$: $Ker((j_p)_{*,e})=Lie(Stab(p))$
	\end{Cor}
	\section{The vertical distribution}
	In this section we look into the canonical vertical distribution of a principal bundle. We have seen in REFERENZA that in any smooth fiber bundle we have a canonical vertical distribution, given by the kernel of the push-forward of the projection. In the case of principal bundles, this vertical distribution is strictly tied to the notion of fundamental vector fields.
	\begin{Obs}
		Recall that on a bundle of any kind we have a canonical vertical bundle:
		$$\mathcal{V}=Ker(\pi_*)$$
		At any point, this spans the vertical space. Note that, in the case of a principal bundle, from REFERENZA, we can say that the verticla distribution is right-invariant:
		$$Ker(\pi_*(r_g)_*)=Ker(\pi_*)$$
	\end{Obs}
	\begin{Obs}
		We can make some easy dimensional consideration, since the vertical distribution is constant rank: at any $p\in P$ we have:
		$$dim(T_pP)=dim(Im(\pi_{*,p}))+dim(Ker(pi_{*,p}))$$
		However, we know that the map $\pi:P\rightarrow M$ is surjective. This implies that:
		$$dim(Im(\pi_*))=dim(T_\pi(p)M)$$
		And by selecting a trivializatio we also know that $dim(G)=dim(Ker(\pi_*))$. Thus, we can say:
		$$dim(T_pP)-dim(T_\pi(p)M)=dim(Ker(\mathcal{V}_p))=dim(G)$$
	\end{Obs}
	\begin{Def}
		We call a vector $X_p\in \mathcal{V}_p$ \textit{vertical}. 
	\end{Def}
	\begin{Prop}\label{fund_vec_are_vert}
		For every element of $P$, the corresponding fundamental tangent vector is vertical.
	\end{Prop}
	\begin{proof}
		From REFERENZA, we just need to prove that the fundamental tangent vector is in the kernel of the differential of $j_p$. Recall that the fundamental vector field is the differential of the map $j_{x}:G\rightarrow E$ defined as 
		$$j_{x}(h)=x\cdot h $$
		Taking now the composition with the projection:
		$$(\pi\circ j_{p})(h)=\pi(p\cdot h)=\pi(p)$$
		This result is independent of the choice of $h\in G$, thus the composition $\pi\circ j_{p}$ is the constant map and so its differential is zero.\\
		Recalling that $\bar{X}_{p}=(j_{p})_*(X)$ we clearly see that:
		$$(\pi_*)_{p}(\bar{X}_{p})=
		((\pi_*)_{p}\circ (j_{p})_*)(X)=
		(\pi\circ j_{p})_*(X)=0$$
	\end{proof}
	From this proposition it follows a key result: the fundamental vector fields are precisely the vertical vector fields.
	\begin{Prop}
		Let $(P,M,\pi,G)$ be a principal $G-$bundle and $p\in P$. Then: 
		$$(j_{p})_{*,e}:\mathfrak{g}\rightarrow \mathcal{V}_{p}$$ is an isomorphism.
	\end{Prop}
	\begin{proof}
		By REFERENZA we know that the kernel of $(j_p)_*$ at the identity is the Lie algebra of the stabilizer of $p$. However, for a principal $G$-bundle, the group acts freely on the manifold $P$ and so the stabilizer is trivial $Stab(p)=\{e\}$. Thus, $Ker ((j_{p})_{*,e})=0$ and the map is injective. From REFERENZA, the image of the differential lies in the vertical subspace. Moreover, $dim(\mathcal{V}_{p})=dim(G)$ and so the map must be an isomorphism of vector spaces.			
	\end{proof}
	\begin{Obs}
		...
		Let $T^a$ be a basis for the Lie algebra $\mathfrak{g}$, then, at any point, by the isomorphism in the previous result, we get that they also form a basis of $\mathcal{V}_{p}$ (under the action of the diffeomorphism). This defines the \textit{Vertical subbundle} $$\mathcal{V}=\bigsqcup_{p\in P}\mathcal{V}_{p}$$
	\end{Obs}
	\section{Connections and horizontal subbundle}
	In this section we finally give the definition of an Ehresmann connection, as we have all of the necessary tools to understand it. Moreover, we will show that such forms are equivalent to the choice of an horizontal right-invariant distribution on the principal bundle.
	\begin{Def}
		We say that $\mathcal{H}$ is an \textit{horizontal distribution} for a principal $G$-bundle $P$ on $M$ if $TP=\mathcal{H}\oplus\mathcal{V}$.
	\end{Def}
	Clearly this means that for every point $x\in E$ we have $T_{p}P=\mathcal{H}_{p}\oplus\mathcal{V}_{p}$ with $\mathcal{H}_{p}\cap\mathcal{V}_{p}=0$. \\\\	In general, there is no canonically defined horizontal distribution on a principal $G$-bundle. This comes from the fact that the complement of $\mathcal{V}_p$ is not unique. We will see that choosing an horizontal distribution means choosing a connection on the principal bundle. \\
	\\
	We have already treated distribution in chapter REFERENZA; however we will fix the following definitions:
	\begin{Def}
		We say that an horizontal distribution $\mathcal{H}$ is \textit{right invariant} if for every $g\in G$, $(r_g)_*(\mathcal{H})\in \mathcal{H}$.
		We say that $\mathcal{H}$ is a \textit{smooth distribution} if it is a smooth sub-bundle of the tangent bundle.
	\end{Def}
	Let $(P,M,\pi,G)$ be a principal $G$-bundle with a vertical and horizontal distribution, so that $TP=\mathcal{H}\oplus\mathcal{V}$. Let $\nu_{p}:\mathcal{H}_{p}\oplus\mathcal{V}_{p}\rightarrow \mathcal{V}_{p}$ be the standard projection on the vertical subbundle. Obviously, if $X_{p}\in T_{p}P$ is a vector field on $P$ at a point $p$, we call $\nu_{p}(X_{p})$ its \textit{vertical component}. The horizontal projector will be referred to as: $\hat{h}:\mathcal{H}\oplus\mathcal{V}\rightarrow \mathcal{H}$
	\begin{Def}\label{Def_6.6}
		We call an \textit{Ehresmann connection}, or \textit{connection on a principal bundle} a $\mathfrak{g}$-valued 1-form $A:TP\rightarrow \mathfrak{g}$ with the properties:
		\begin{itemize}
			\item Given any $X\in\mathfrak{g}$ and $p\in P$ we have $A_{p}(\bar{X}_{p})=X$;
			\item $G$-equivariance for $g\in G$ we have $r^*_g\omega=\hbox{Ad}(g^{-1})A$;
			\item $A$ is $C^\infty$.
		\end{itemize}
	\end{Def}
	...
	$$A_{p}=(j_{p})_*^{-1}\circ \nu:T_{p}P\xlongrightarrow{\nu} \mathcal{V}_{p}\xlongrightarrow{(j_{p})_*^{-1}}\mathfrak{g}$$
	This map takes a vector, selects its horizontal component and maps it to the corresponding Lie algebra element.
	\begin{Theo}
		If $\mathcal{H}$ is a smooth right-invariant horizontal distribution on a principal $G$-bundle $(P,M,\pi,G)$, then the 1-form $A_{p}=dj_{p}^{-1}\circ \nu_p$ is an Ehresmann connection.
	\end{Theo}
	\begin{proof}
		We have to prove the 3 properties in REFERENZA.\\
		Consider any $X\in\mathfrak{g}$ and any $p\in P$. Then the fundamental vector field in this point is in the vertical sub-bundle and so the projection $\nu$ leaves it invariant:
		$$A_{p}(\bar{X}_{p})=((j_{p})_*^{-1}\circ \nu)(\bar{X}_{p})=((j_{p})_*^{-1}(\bar{X}_{p})=
		((j_{p})_*^{-1}(dj_{p}(X))=X$$
		This proves the first property.\\
		\\
		As for the second property, we recall that in REFERENZA we proved that for a fundamental vector field  we have:
		$$(r_g)_*(\bar{X})=\overline{\hbox{Ad}(g^{-1})(X)}$$
		If $\overline{X_p}$ is a vertical vector, we need to prove that:
		$$r^*_h\circ A_{p}(\overline{X_{p}})=\hbox{Ad}(h^{-1})A_{p}(\overline{X_{p}})$$
		The pullback of a 1-form gives us:
		$$r^*_h\circ A_{p}(\overline{X_{p\cdot h}})=A_{p\cdot h}((r_h)_*(\overline{X_{p}}))$$
		If $X_{p}\in\mathcal{V}$ is vertical then it will be a fundamental vector field of an element $X\in\mathfrak{g}$ and so by REFERENZA we get:
		$$A_{p\cdot h}((r_h)_*(\overline{X_{p}}))=A_{p\cdot h}\overline{(\hbox{Ad}(h^{-1})(X_{p\cdot h}))}$$
		Now applying the previous property twice:
		$$A_{p\cdot h}(\overline{\hbox{Ad}(h^{-1})(X_{p\cdot h})})=\hbox{Ad}(h^{-1})(X)=\hbox{Ad}(h^{-1})A_{p}(\overline{X_{p}})$$
		If instead $X_{p}$ is horizontal then by the invariance horizontal distribution the field $r_*(X)$ will still be horizontal ad so it will be annihilated by $A$ since it contains the projection on the vertical subbundle.\\\\
		As for the last property, we need to prove that $A$ is smooth in a neighborhood of an arbitrary point $p\in P$. Since $P$ is a manifold, we choose a chart $(U_P,\phi_P)$ centered in a point $p\in P$. Let $\{T^i\}$ be a basis of the Lie algebra $\mathfrak{g}$ and $\{\bar{T^i}\}$ the associated fundamental vectors. Those fundamental vectors are all smooth by proposition REFERENZA. By the previous isomorphism, those span the whole vertical sub-bundle at any point. Furthermore, since by hypothesis $\mathcal{H}$ is a smooth distribution of horizontal vectors, there is a basis of smooth vector fields $X^i$ in any point that spans the whole space. Thus, since $TP=\mathcal{H}\oplus\mathcal{V}$, we can expand any element of $TP$ as a linear combination:
		$$V=a_i \bar{T^i}+b_iX^i;\hbox{ with } V\in TP, a_i,b_i \hbox{ functions}$$
		The coefficients $a_i,b_i$ are smooth by the previous observations. Now, applying $A$, we get that the horizontal components are annihilated:
		$$A(a_i \bar{T^i}+b_iX^i)=A(a_i \bar{T^i})=a_i T^i$$
	\end{proof}
	...
	\\
	The correspondence goes both ways: if one can find a smooth $G$-equivariant 1-form that takes value on the Lie algebra $\mathfrak{g}$ of $G$ such that $A(\bar{X})=X$, then one can construct the horizontal distribution. In particular, one defines: $$\mathcal{H}_{p}=Ker(A_{p})$$
	\begin{Prop}\label{Prop_5.3.1}
		If $\mathcal{H}$ is a smooth right-invariant distribution on a principal $G$-bundle $E$, then $\forall g\in G$, the map $(r_g)_*:TP\rightarrow TP$ commutes with the horizontal and the vertical projection operators.
	\end{Prop}
	\begin{proof}
		Let $\hat{h}:TP\rightarrow\mathcal{H}$ be the horizontal projection and $p\in P$. Since both the horizontal and the vertical distributions are right-invariant, it means that for any $X_{p}\in T_pP$ we have $(r_h)_*( X_{p})=(r_h)_* \nu(X_{p})+(r_h)_*\hat{h}(X_{p})$ and each of the two terms is still vertical/horizontal, so that:
		$$(r_h)_*(X_{p})=
		\nu\circ (r_h)_*(X_{p})+\hat{h}\circ (r_h)_*(X_{p})$$
	\end{proof}
	We ought to prove the next result:
	\begin{Theo}
		If $A$ is an Ehresmann connection, $Ker(A)=\mathcal{H}$ is a smooth right-invariant horizontal distribution on $TP$.
	\end{Theo}
	\begin{proof}
		PROVAREEE
	\end{proof}
	\section{The horizontal Lift of a vector field}
	In this section we introduce the notion of horizontal lift of a vector field and look at some properties of it. The idea is the same behind the horizontal lift in the case of vector bundles. However, I cannot stress enough how this procedure appears more general in the case of principal bundles.\\
	\\
	Suppose that $\mathcal{H}$ is a horizontal distribution on a principal bundle $(P,M,\pi,G)$. Let $X\in\mathfrak{X}(M)$ be a vector field on $M$. Then for every point $p\in P$, we have $\mathcal{V}_{p}=Ker((\pi_{p})_*)$ and so we have some induced isomorphisms:
	\begin{equation}\label{vert_iso}
		{T_{p}P\over \mathcal{V}_{p}}\simeq \mathcal{H}_{p}\simeq T_{\pi(p)}M
	\end{equation}
	\begin{Def}
		Let $(P,M,\pi,G)$ be a principal $G$-bundle with a smooth right-invariant horizontal distribution $\mathcal{H}$. Let $X$ be a vector field on $M$. We call \textit{lift} of $X$ to $P$ any vector field $\tilde{X}$ such that $\pi_*(\tilde{X})=X$. We call the unique horizontal vector field $\mathcal{X}\in \mathcal{H}$ in the decomposition $TP=\mathcal{V}\oplus\mathcal{H}$, such that $\pi_*(\mathcal{X})=X$ the \textit{horizontal lift} of $X$ to $P$.
	\end{Def}
	\begin{Obs}
		Given any principal $G$-bundle on $M$ with a horizontal distribution, we see from the previous series of isomorphisms that the tangent space of the basis manifold $M$ can be identified with the horizontal tangent space of the principal bundle, via the isomorphism REFERENZA.
	\end{Obs}
	...
	\begin{Prop}
		Let $(P,M,\pi,G)$ be a principal bundle with a smooth right-invariant horizontal distribution $\mathcal{H}$. Then the horizontal lift $\mathcal{X}$ of any vector field $X\in\mathfrak{X}(M)$ is smooth and right-invariant as well.
	\end{Prop}
	\begin{proof}
		Suppose that $x\in M$ and $p\in\pi^{-1}(x)$. Then we must have by definition of lift: $\pi_*(\mathcal{X}_{p})=X_x$. Suppose now to take any other point $q\in\pi^{-1}(x)$ with $p\neq q$. Then, we must have, by REFERENZA, that there exists $g\in G$ such that $q=(r_{g})_*p=p\cdot g$. Recalling that, by observation REFERENZA, $\pi\circ r_{g}=\pi$ we have:
		$$\pi_*\circ (dr_{g})_*(\mathcal{X}_{p})=\pi_*\mathcal{X}_{p}=\pi_*\mathcal{X}_{q}$$
		By uniqueness of the horizontal lift, $\mathcal{X}_{p}=\mathcal{X}_{q}$ and this proves right-invariance.\\
		\\
		...
		$$Z_{(x,g)}=(X_x,0)\in T_{(x,g)}(U\times G)$$
		...
		$$\pi_*(Y_{p})=X_{\pi(p)}$$
		We know that the projection $\hat{h}(Y)$ is smooth by hypothesis, If now we decompose $Y_{p}=\nu(Y_{p})+\hat{h}(Y_{p})$, recalling that $\mathcal{V}=Ker(\pi_*)$, we get:
		$$\pi_*(Y_{p})=\pi_*(\hat{h}(Y_{p}))=\hat{h}(\pi_*(Y_{p}))=X_{\pi(p)}$$
		This means that the smooth vector field $\hat{h}(Y)$ is the unique horizontal lift of $X$.
	\end{proof}
	\section{Curvature on a principal bundle}
	In this section we focus on the curvature form. Many things are to be said. Physically, the curvature fomr will be trictly tied to the concept of energy of our system. As for the geometricla point of view, it musst be somewhat clear by now, that the connections we have been defining on principal bundles must somehow descent to some vector bundle framework. In particular, recalling the constructions of chapter REFERENZA, we have already defined curvature and connections in such a fashion. We were also able to describe those in terms of matrices i.e. forms. However, this description was tied to some locality i.e. to frames. The framework of principal bundles will have no such restrictions.
	\begin{Def}
		Let $(P,M,\pi,G)$ be a principal $G$-bundle and $A$ an Ehresmann connection on it. We define the \textit{curvature form} of $A$ as:
		$$F=dA+{1\over 2}[A,A]$$
	\end{Def}
	Clearly, by definition of exterior derivative and by extension of the commutator on the space of $\mathfrak{g}$-valued forms, $F\in \Omega^2(P,\mathfrak{g})$. 
	\begin{Theo}
		The following properties hold:
		\begin{itemize}
			\item the curvature is \textit{horizontal}: for every $X_p,Y_p\in T_pP$,
			$$F_p(X_p,Y_p)=dA_p(\hat{h}(X_p),\hat{h}(Y_p))$$
			\item $F$ is \textit{$G$-equivariant}: for every $g\in G$ we have $r^*_g(F)=\hbox{Ad}(g^{-1})(F)$;
			\item the curvature respects the \textit{second Bianchi identity}: $dF=[F,A]$.
		\end{itemize}
	\end{Theo}
	\begin{proof}
		To prove the first property, let us consider all of the possible cases:
		\begin{itemize}
			\item \textit{case 1}: the vectors are horizontal. Clearly in this case $F_p(X_p,Y_p)=dA(X_p,Y_p)$ since the connection annihilates all of the horizontal components.
			\item \textit{case 2}: one vector is horizontal and the other is vertical. Then:
			$$F_p(X_p,Y_p)=F_p(\hat{h}(X_p),\nu(Y_p))=$$
			$$dA_p(\hat{h}(X_p),\nu(Y_p))+{1\over 2}[A_p,A_p](\hat{h}(X_p),\nu(Y_p))$$
			The second term vanishes since the connection annihilates the horizontal components. As for the first term: one can prove that if $A$ is a $1$-form, then:
			$$dA(X,Y)=XA(Y)-YA(X)-A([X,Y])$$
			as in REFERENZA.\\
			Recall that any vertical vector field is a fundamental vector field: $Y_p\rightarrow \overline{Y}_p$. In our case, the first term is 0 since $A(Y_p)=A(\overline{Y}_p)=Y\in\mathfrak{g}$ by the defining properties of the connection. Moreover, $X_pY=0$ since its argument is a constant function of $p$. The second term vanishes because $X_p$ is horizontal. As for the first term, it vanishes as well since the commutator between a horizontal and a vertical vector field is 0 from REFERENZA.
			\item\textit{case 3}: both vectors are vertical. By properties of the connection $1$-form:
			$$F(X_p,Y_p)=dA_p(\overline{X_p},\overline{Y_p})+{1\over 2} ([A(\overline{X}),A(\overline{Y})]-[A(\overline{Y}),A(\overline{X}_p)])=$$
			$$=dA_p(\overline{X_p},\overline{Y_p})+[X,Y]$$
			Moreover:
			$$dA(\overline{X}_p,\overline{Y}_p)=\overline{X}_pA(\overline{Y}_p)-\overline{X}_pA(\overline{Y}_p)-A([\overline{X}_p,\overline{Y}_p])$$	
			...
			$$dF_p(X_p,Y_p)=-[X,Y]+[X,Y]=0$$
			This proves the first property.
		\end{itemize}
		As for the second property, it is just a straightforward calculation:
		$$r^*_g(F)=r^*_g(dA)+r^*_g({1\over 2}[A,A])$$
		By proposition REFERENZA:
		$$r^*_g(F)=(r^*_gA)+{1\over 2}[r^*_gA,r^*_gA]=\hbox{Ad}(g^{-1})A+{1\over 2}[\hbox{Ad}(g^{-1})A,\hbox{Ad}(g^{-1})A]=\hbox{Ad}(g^{-1})F$$
		The third property can be found by a simple expansion:
		$$dF=d^2A+{1\over 2}([dA,A]-[A,dA])=[dA,A]=[F-{1\over 2}[A,A],A]=$$
		$$=[F,A]-{1\over 2}[[A,A],A]=[F,A]$$
	\end{proof}
	\chapter{Homology}
	This chapter is very abstract. However, in my personal opinion, it is the duty of a theoretical physicist not only to understand the gist of the mathematical construction behind the theories, but also to be able to relate with matematicians in a meaningful way i.e. to understand their nonsense (I have a PhD in mathematics I can say whatever I want). In the course of this chapter we will dive into the very basic construction of homology and cohomology of a topological space. This construction will then be applied to our geometrical setting. Pleasae be aware that, due to lack of space and time, this analysis will not generalize at the categorical level. I will sneak here and there some references and propositions regarding category theory. Those will however be unnecessary for the full comprehension of the topics exposed.
	\section{Chain and co-chain complexes}
	In this section we will give the general definition of complexes and we will see some properties and important results regarding them.
	\begin{Def}
		Let $R$ be a ring and $K=\{K_n,\partial_n\}$ be a family of $R$-modules together with an homomorphism $\partial_n:K_n\rightarrow K_{n-1}$ such that:
		$$\partial_n\circ \partial_{n+1}=0$$
		\begin{center}
			$$\begin{tikzcd}
				...&K_ {n-1}\arrow[swap]{l}{\partial_{n-1}}&K_ {n}\arrow[swap]{l}{\partial_{n}}&K_ {n-1}\arrow[swap]{l}{\partial_{n+1}}&...\arrow[swap]{l}{\partial_{n+2}}\\
			\end{tikzcd}$$
		\end{center}
		We call $K$ a \textit{chain complex}.
	\end{Def}
	For obvious reasons, we will refer to the property $\partial_n\circ \partial_{n+1}=0$ with \textit{nilpotency}.
	\begin{Def}
		Given a chain complex $K$, we define its \textit{homology} as:
		$$H(K):=\{H_n(K)=Ker(\partial_{n})/Im(\partial_{n+1})\}$$
	\end{Def}
	Clearly, an element of $H_n(K)$ is an equivalence class of "closed but not exact" stuff. We will refer to $c_n\in K_n$ with the word \textit{cycle} if $\partial_n c_n=0$ i.e. if it is closed.
	\\\\
	It is useful, before making examples, to see some other definitions and properties:
	\begin{Def}
		Let $K,K'$ be chain complexes. We define a \textit{chain transformation} $f:K\rightarrow K'$ as a family of maps:
		$f:=\{f_n:K_n\rightarrow K_n'\}$ such that:
		\begin{itemize}
			\item[(i)] $f_n$ is an homeomorphism for all $n$;
			\item[(ii)] $f_{n-1}\circ \partial_n=\partial'_{n}\circ f_n$ i.e. the following diagram commutes:
			\begin{center}
				$$\begin{tikzcd}
					...&K_ {n-1}\arrow[swap]{l}{\partial_{n-1}}\arrow{d}{f_{n-1}}&K_ {n}\arrow[swap]{l}{\partial_{n}}\arrow{d}{f_{n}}&K_ {n-1}\arrow[swap]{l}{\partial_{n+1}}\arrow{d}{f_{n+1}}&...\arrow[swap]{l}{\partial_{n+2}}\\
					...&K'_ {n-1}\arrow[swap]{l}{\partial'_{n-1}}&K'_ {n}\arrow[swap]{l}{\partial'_{n}}&K'_ {n-1}\arrow[swap]{l}{\partial'_{n+1}}&...\arrow[swap]{l}{\partial'_{n+2}}\\
				\end{tikzcd}$$
			\end{center}
		\end{itemize}
	\end{Def}
	\begin{Prop}
		A chain transformation defines a mapping: 
		$$H_n(f):H_n(K)\rightarrow H_n(K')\hbox{ like } H_n(f)[c_n]=[f_n(c_n)]$$
	\end{Prop}
	\begin{proof}
		This is not hard. Clearly, if $c_n+\partial_{n+1}c_{n+1}$ is a generic element of $[c_n]$, then:
		$$H_n(f)(c_n+\partial_{n+1}c_{n+1})=f_nc_n+f_n\partial_{n+1}c_{n+1}$$
		due to the properties of the chain transformation we can commute the operator $\partial$ and obtain:
		$$H_n(f)[c_n]=f_nc_n+\partial'_{n+1}f_{n+1}c_n+1$$
		This makes it clear that the map is independent of the representative.
	\end{proof}
	\begin{Cor}
		Consider the category of chain complexes $\mathbf{Ch}_R$ over the module $R$ as objects and chain transformations as arrows. Then, $H_n:\mathbf{Ch}_R\rightarrow \textbf{Mod}_R$ is a covariant functor, obviously.
	\end{Cor}
	This is pretty straightforward.
	\begin{Def}
		A \textit{chain homotopy} between two given chain transofrmations $f,g:K'\rightarrow K'$ is defined as a family of homomorphisms $s_n:K_n\rightarrow k'_{n+1}$ such that:
		$$f_n-g_n=\partial'_{n+1}\circ s_n-s_{n-1}\circ\partial_n$$
		In this case we say $f\sim g$ through $s$.
	\end{Def}
	\begin{Prop}
		Let $s_n$ be a chain homotopy between two chain transformations $f,g$. Then $H(f)=H(g)$.
	\end{Prop}
	\begin{proof}
		Since the maps $H_n$ are well defined, take a cycle of $[c_n]$ as a representative and evalue the difference:
		$$(H_n(f)-H_n(g))[c_n]=[f_nc_n-g_nc_n]=(\partial'_{n+1}\circ s_n-s_{n-1}\circ\partial_n)c_n$$
		Due to $c_n$ being a cycle, it gets sent to $0$ by the $\partial$ operator. As for the other term, it is clearly exact. Thus:
		$$(H_n(f)-H_n(g))[c_n]=[0]$$
	\end{proof}
	\begin{Def}
		A \textit{chain equivalence} $f:K\rightarrow K'$ is a chain transofmration with an inverse i.e. such that there is another chain transformation $f^{-1}:K'\rightarrow K$ and two homotopies $s,t$ which make:
		$$f\circ f^{-1}\sim\mathbb{I}_K'\hbox{ and }f^{-1}\circ f\sim\mathbb{I}_K$$
	\end{Def}
	\begin{Prop}
		Let $f:K\rightarrow K'$ be a chain equivalence. Then the induced map $H_n(f)$ is an isomorphism for all $n$.
	\end{Prop}
	\begin{proof}
		This is obvious from the previous result REFERENZA.
	\end{proof}
	\section{The exact homology sequence}
	In this section we will ho through some important results regarding homology. Those are incredibly important result, as we wilò brutally generalize them in the case of the cohomology, thanks to the extension being natural.
	\begin{Def}
		Let $K,M,L$ be chain complexes and let $f:K\rightarrow L$ and $g:L\rightarrow M$ be chain transformations. We say that this induces a \textit{short exact sequence of homologies} if:
		\begin{itemize}
			\item[(i)] for all $n$, $Ker(g_n)=Im(f_n)$;
			\item[(ii)] the following diagram commutes:
				\begin{center}
				$$\begin{tikzcd}
					&...\arrow{d}&...\arrow{d}&...\arrow{d}&\\
					0\arrow{r}&K_{n+1}\arrow{r}{f_{n+1}}\arrow{d}{\partial^K_{n+1}}&L_{n+1}\arrow{r}{g_{n+1}}\arrow{d}{\partial^L_{n+1}}&M_{n+1}\arrow{r}\arrow{d}{\partial^M_{n+1}}&0\\
					0\arrow{r}&K_{n}\arrow{r}{f_{n}}\arrow{d}{\partial^K_{n}}&L_{n}\arrow{r}{g_{n}}\arrow{d}{\partial^L_{n}}&M_{n}\arrow{r}\arrow{d}{\partial^M_{n}}&0\\
					0\arrow{r}&K_{n-1}\arrow{r}{f_{n-1}}\arrow{d}{\partial^K_{n-1}}&L_{n-1}\arrow{r}{g_{n-1}}\arrow{d}{\partial^L_{n-1}}&M_{n-1}\arrow{r}\arrow{d}{\partial^M_{n-1}}&0\\
					&...&...&...&\\
				\end{tikzcd}$$
			\end{center}
	...
	We write:
		\begin{center}
		$$\begin{tikzcd}
			0\arrow{r}&K\arrow{r}{f}&L\arrow{r}{g}&M\arrow{r}&0\\
		\end{tikzcd}$$
	\end{center}
	The use of pedant notation in the denomination of the $\partial$ operators is understood.
	\begin{Theo}
		Let $K,L,M$ be chain complexes and $$\begin{tikzcd}
			0\arrow{r}&K\arrow{r}{f}&L\arrow{r}{g}&M\arrow{r}&0\\
		\end{tikzcd}$$ be a short exact sequence as above. Then there is a long-exact sequence following the diagram below:
		\begin{center}
			\begin{tikzpicture}[descr/.style={fill=white,inner sep=1.5pt}]
				...
				{
					& \dots & H_{n+1}(M) \\ 
					H_{n}(K) & H_{n}(L) & H_{n}(M) \\
					H_{n-1}(K) & \dots & {} \\ 
				};
				
				\path[overlay,->, font=\scriptsize,>=Latex]
				(m-1-2) edge node[yshift=0.8ex]{$g_*$} (m-1-3)
				(m-1-3) edge[out=355,in=175] node[descr,yshift=0.3ex] {$\partial_*$} (m-2-1)
				(m-2-1) edge node[yshift=0.8ex]{$f_*$} (m-2-2)
				(m-2-2) edge node[yshift=0.8ex]{$g_*$} (m-2-3)
				(m-2-3) edge[out=355,in=175] node[descr,yshift=0.3ex] {$\partial_*$} (m-3-1)
				(m-3-1) edge (m-3-2);
			\end{tikzpicture}
		\end{center}
		where $f_*,g_*$ are the induced mappings of the cohomologies as in REFERENZA, while $\partial_*$ is defined as:
		$$\partial_*=cls_K\circ f^{-1}\circ \partial \circ g^{-1}\circ cls_M^{-1}$$
	\end{Theo}
	\begin{proof}
		The proof is lengthy and boring. But useful. Notice that a-priori there is no well defined inverses for our maps, yet we have used them in the formula for $\partial_*$. This means there should be independence of the pre-images. We call $\partial_*$ the \textit{connecting homomorphism}. We construct it as follows: take any element $m_{n}+\partial M_n\in M_n$. Since $g$ is surjective, there will be an element of $L_n$ such that: $g(l_n)=m_{n}+\partial M_n$. However, since $g$ commutes with the $\partial$ operator: 
		$$g_{n-1}(\partial^L_nl_n)=\partial_{n}^M(m_{n}+\partial M_n)=0$$
		This means that $\partial^L_nl_n\in Ker(g)$. Since we have a short exact sequence, this coincides with $Im(f)$. Thus, there will be some $c_{n-1}\in K_{n-1}$ such that $f(c_{n-1})=\partial^L_nl_n$. Moreover, since $f$ is an injective map: $\partial_{n-1}^L f(c_{n-1})=0$ implies that this $c_{n-1}$ is unique. We set $\partial_*[m_n]=c_{n-1}$.\\
		\\
		We have to prove a multitude of things.
		\begin{itemize}
			\item[1] $Ker(g_*)=Im(f_*)$\\
			\\
			This is obvious by the action of $f_*$ and $g_*$.
			\item[2] $Ker(\partial_*)=Im(g_*)$\\
			\\
			Consider any $[l_n]\in H_n(L)$. We have:
			$$\partial_* g_*[l_n]=\partial_* [g(l_n)]$$
			Take $l_n$ to be a cycle, so that $\partial_nl_n=0$. This way, by applying $\partial_*$ in the definition above, we have:
			$$\partial_*[g(l_n)]=cls_K\circ f^{-1}\circ \partial l_n=0$$
			\item[3] $Ker(f_*)=Im(\partial_*)$\\
			\\
			Take $\partial_* [m_n]$, with $m_n$ a cycle. We automatically have:
			$f_*\partial_*[m_n]=f_*[c_{n-1}]=[\partial^L_nl_n]=0$.
		\end{itemize}
	\end{proof}
	This was nice. There is also another nice thing happening in the back with this homomorphism:
	\begin{Prop}
		The connecting homomorphism is natural i.e. the following diagram commutes:
		
	\end{Prop}
	\section{Singular homology}
	In this section we take the previous definition of homology and apply it to a very generla setting: a topological space. The idea is to be able to associate to any topological space a chain complex. 
	\begin{Def}
		Consider the set:
		$$\Delta^n=\big\{\vec{x}\in\mathbb{R}^{n+1}\hbox{ such that }\sum_i x^i=1\hbox{ and } x^i\geq 0\big\}$$
		We define a $n$-simplex on $X$ as any continuous map:
		$$T:\Delta^n\rightarrow X$$
	\end{Def}
	\begin{Obs}
		Accoarding to the previous definition there is a very natural interpretation:
		a 0-simplex is just a point, a 1-simplex is a line and so on...
	\end{Obs}
	\begin{Obs}
		There is an obvious operator on the space of $n$-simplexes. Consider:
		$$\epsilon_n^i:\Delta^{n-1}\rightarrow \Delta^{n}\hbox{ like }\epsilon_n^i(x^0,...,x^n)=(x^0,...,x^{i-1},0,x^i,...,x^n)$$
		Basically $\epsilon$ embeds the $n-1$-simplex domain into the corresponding "face" of the higher dimensional simplex domain. This defines the following mapping:
		$$d_iT=T\circ \epsilon_n^i\Delta^{n-1}\rightarrow X$$
		This is clearly still a $n$-simplex.
	\end{Obs}
	This operator is however ill-defined, in the sense that it does not follow the nice properties of a chain complex framework. Consider the following lemma:
	\begin{Lm}
		$d_id_jT=d_jd_iT$ if $i>j$ while $d_id_jT=d_{j-1}d_iT$ for $i<j$.
	\end{Lm}
	\begin{proof}
		This is not hard and is suggested as a brief exercise for the reader. The proof is the following: take $i<j$, then
		$$d_id_jT=T\circ\epsilon_n^j\circ\epsilon_n^i$$
		The first operator adds a $0$ in between the $i-1$ and the $i$ coordinates. The second adds a $0$ in between the $j-1$ and the $j$. However, for $i<j$, once we apply $d_i$, the coordinates $k>i$ shift upwards by 1. This proves the claim. 
		\\
		The case $i>j$ is obvious.
	\end{proof}
	\begin{Obs}
		We can consider the free abelian group generated by all of the $n$-simplices:
		$$S_n(X)=\big\{a^iT_i\hbox{ with }a^i\in \mathbb{Z}\big\}$$
		Clearly, the operators $d_i:S_n(X\rightarrow S_{n-1}(X))$ are homomorphisms. We define:
		$$\partial_n=(-)^id_i\hbox{ with understood Einstein notation}$$
		In particular:
		$$\partial_n T=d_0T-d_1T+...+(-)^nd_nT$$
		This is clearly an homomorphism:
		$$\partial_n:S_n(X)\rightarrow S_{n-1}(X)$$
	\end{Obs}
	\begin{Prop}
		Let the notation be as immediately above. Then:
		$$\partial_n\circ \partial_{n+1}=0$$
	\end{Prop}
	\begin{proof}
		The proof is a straightforward calculation.
		$$\partial_n\circ \partial_{n+1}T=\sum_{i,j=0}^n (-)^{i+j}d_id_jT=\big[\sum_{i<j}(-)^{i+j}+\sum_{i\geq j}(-)^{i+j}\big]d_id_jT=$$
		$$=\sum_{j-1\geq i}(-)^{i+j}d_{j-1}d_i+\sum_{i\geq j}(-)^{i+j}d_jd_iT=0$$
		Where we have used the fact that the last two sums, after a renaming of the indices, are clearly opposite to one another.
	\end{proof}
	\begin{Def}
		Let $X$ be a topological space. We define the \textit{singular chain-complex} of $X$ as:
		$$S(X)=\{S_n(X),\partial_n\}$$
		We also define the \textit{singular $n$-homology} of $X$ as:
		$$H_n(X)={Ker\partial_n\over Im\partial_{n+1}}$$
	\end{Def}
	Clearly, since this is in general a chain complex, all of the previous results still hold clear. 
	\begin{Prop}
		Continuous functions are chain transformations.
	\end{Prop}
	\begin{proof}
		Thie amounts to show that $f:X\rightarrow Y$ a continuous map commutes with the derivative operator. But this is easy: the "action" of a contuinuous map on an element of $S_n(X)$ is just the composition: $f(T_n)=f\circ T_n$
		$$\partial_n^Y\circ f(T_n)=\sum_i(-)^if\circ T_n\circ \epsilon_n^i=f\circ \partial^X_n (T_n)$$
	\end{proof}
	\begin{Prop}
		There is a covariant functor $H_n:\mathbf{Top}\rightarrow \mathbf{Ab}$
	\end{Prop}
	\begin{proof}
		This is clear from REFERENZA but nevertheless we make a little proof. Take any continuous map $f:X\rightarrow Y$ while recalling that morphisms of the $\mathbf{Top}$ categroy are indeed continuous maps, and define:
		$$H_n(f):H_n(X)\rightarrow H_n(Y)\hbox{ like $H_n(f)[c_n]=[f(c_n)]$}$$
	\end{proof}
	LAstly, before actually applying all of this abstract nonsense, we remark the homotopy thingy:
	\begin{Def}
		Two continuous maps $f,g:X\rightarrow Y$ are said to be \textit{homotopic} if there is a continuous map $F:X\times [0,1]\rightarrow Y$ such that
		$$F(x,0)=f(x)\hbox{ and }F(x,1)=g(x)$$
	\end{Def}
	\begin{Prop}
		If two continuosu maps $f,g:X\rightarrow Y$ are homotopic, then the corresponding chain transformations are homotopic as well.
	\end{Prop}
	\begin{proof}
		To see this 
	\end{proof}
	\section{Homology properties and the Maier-Vietoris sequence}
	In this section we calculate the homology of some topological spaces.
	\begin{Prop}
		Let $X$ be the topological space made of only one point. Then $H_0(X)\simeq\mathbb{Z}$ and $H_n(X)\simeq0$ for all $n\geq 1$.
	\end{Prop}
	\begin{proof}
		Take any $n\geq 0$. Since the space $X$ has only one point, for all $n$ there is only one map:
		$$T^n:\Delta^n\rightarrow X\hbox{ like }T^n(\vec{t})=x\in X$$
		This means that, by definition:
		$$S_n(X)=\mathbb{Z}\braket{T^n}\simeq \mathbb{Z}$$
		It is also clear that, since:
		$$\partial_n T^n=\sum_{i=0}^n(-)^iT^{n-1}$$
		the result is strongly dependent on n: if $n$ is even the result is 0, if it is odd the result is 1. Thus:
		$$Im(\partial_n)=0,1\hbox{ depending on }n$$
		For $n=0$, $\partial_0$ is the 0 map, and so $Ker(\partial_0)=\mathbb{Z}$. This means:
		$$H_0(X)=Ker(\partial_0)/Im(\partial_1)$$
		Moreover, $Im(\partial_1)=0$. This means:
		$$H_0(X)\simeq\mathbb{Z}$$
		As for the others:
		\begin{itemize}
			\item for $n>0$ even, the operator $\partial_n$ is the  map $T_n\rightarrow T_{n-1}$, so $Ker(\partial_n)=0$ which implies:
			$$Im(\partial_{n+1})=\mathbb{Z}$$
			Thus:
			$$H_n(X)=0$$
			\item for $n>0$ odd instead, we have the contrary: $\partial_n$ is the 0 map, so its kernel is the whole $\mathbb{Z}$. On the other hand, $\partial_{n+1}$ is the $T_{n+1}\rightarrow T_{n}$ map, so its image is still $\mathbb{Z}$. Thus:
			$$H_n(X)=\mathbb{Z}/\mathbb{Z}=0$$ 
		\end{itemize}
	\end{proof}
	\begin{Cor}
		If $X$ is contractible REFERENZA, then Then $H_0(X)\simeq\mathbb{Z}$ and $H_n(X)\simeq0$ for all $n\geq 1$.
	\end{Cor}
	This follows immediately from REFERENZA homotopy invariance.
		\begin{Prop}
			If $K$ is a complex and $\{K^i\}_{i=1,...,m}$ are all of the connected components of $K$, then:
			$$H_n(K)=H_n(K^1)\oplus...\oplus H_n(k^m)$$
		\end{Prop}
		\begin{proof}
			The idea is to show that there is a similar decomposition for the two sets we are quotienting.
			Consider a fixed $n$ and the chains $K_n^i$ with $\partial_i$. Clearly, by construction:
			$$K_n=K^1\oplus...\oplus K^m$$
			Thus, we can make the following decomposition:
			$$Im(\partial_{n+1})=Im(\partial_{n+1})\big|_{K^1}\oplus...\oplus Im(\partial_{n+1})\big|_{K^m}$$
			The same goes for the Kernel. Take $c_{n+1}\in K_{n+1}$ and its decomposition:
			$$c_{n+1}=c^1_{n+1}+...+c_{n+1}^m$$
			It is clear that: $\partial_{n+1}c_{n+1}=0$ automatically implies $\partial_{n+1}c_{n+1}^i=0\forall i$. This means that the kernel decomposes:
			$$Ker(\partial_n)=Ker(\partial_n)\big|_{K^1}\oplus...\oplus Ker(\partial_n)$$
			This proves the claim.
		\end{proof}
	\begin{Prop}
		Let $K$ be a homology of finitely generated abelian groups. Then $H(K)$
	\end{Prop}
	\begin{Ex}
		Consider the circle $S^1$.
	\end{Ex}
	\chapter{Cohomology}
	In this section we construct the cohomology of a given homology. In general, this is a dual construction. For a physicist, used to work with de Rham cohomology, this might seem weird: why start from the homology? The idea is to generalize the construction as much as possible. This way, we will be able to show that the de Rham complex is actually a cohomology: the canonical one of the underlying smooth manifold. Substantially, this intricate homology-cohomology construction is there to see the deep connection between geometry and physics. 
	\begin{Def}
		Let $K$ be a chain complex and let $G$ be...
	\end{Def}
	...
	\section{Complex Clifford algebra's structure}
	In this section we go through an analysis of the structure of the complex Clifford algebras.
	\begin{Lm}
		Complex Clifford algebras are periodic:
		$$\mathbb{C}l(n+2)\simeq \mathbb{C}l(n)\otimes_{\mathbb{C}}End(\mathbb{C}^2)$$
	\end{Lm}
	\begin{proof}
		Let $\{e_1,...,e_n\}$ be the standard base of $\mathbb{C}^n$. Define $(x,y)\in\mathbb{C}^n\oplus\mathbb{C}^2$, then:
		$$\delta:\mathbb{C}^{n+2}\rightarrow \mathbb{C}l(n)\otimes_{\mathbb{C}}\mathbb{C}l(2)$$
		$$\delta(x,y)=[x\otimes (-ie_1e_2)]+[1\otimes y]$$
		Clearly, $\omega=-ie_1e_2$ is the chirality element for $Cl(2)$, so that:
		$$\delta(x,y)^2=x^2\otimes \omega^2+1\otimes y^2+x\otimes \omega y+x\otimes y\omega$$
		Clearly, $\omega^2=1$ and $\{\omega,y\}=0$, so that:
		$$\delta(x,y)^2=x^2\otimes 1+1\otimes y^2=Q(x)+Q(y)$$
		This implies that the anticommutator satisfies:
		$$\{\delta(x,y),\delta(x,y)\}=2\braket{x,y}$$
		From the universal property REFERENZA, there is a unique homomorphism:
		$$\phi:\mathbb{C}l(n+2)\rightarrow Cl(n)\otimes \mathbb{C}l(2)$$
		Notice now that the products of the images of $\delta(x,y)$ clearly span the whole space $\mathbb{C}l(n)\otimes \mathbb{C}l(2)$ (take one element to be 0 at a time).
		This means that, since the spaces have same dimension and the map is surjective, the map is also an isomorphism.\\
		\\
		The fact that $End(\mathbb{C}^2)\simeq Cl(2)$ follows from REFERENZA.
	\end{proof}
	\begin{Lm}
		For any $n\geq 1$, we have:
		$$Cl^{even}(n)\simeq Cl(n-1)$$
	\end{Lm}
	\begin{proof}
		Consider the map:
		$$f:\mathbb{C}^{n-1}\rightarrow {Cl}^{even}(n)\hbox{ like }x\rightarrow x\cdot e_n$$
		This map satisfies $$f(x)^2=x\cdot e_n\cdot x\cdot e_n=x^2=Q(x)$$
		Thus, by the same means of the above proof, the isomorphism is proved.
	\end{proof}
	\begin{Theo}
		The following decompositoon holds for complex valued Clifford algebras:
		TABELLAAAAA
	\end{Theo}
	\begin{proof}
		Take $\mathbb{C}l(n)$ with $n$ even. Then, we can use REFERENZA to split it. We go by induction. Consider: $\mathbb{C}l(0)=\mathbb{C}$ and suppose that $\mathbb{C}l(n)\simeq End(\mathbb{C}^{2^{n/2}})$. Writing $n$ as $2k$, we can prove this for $k+1$:
		$$\mathbb{C}l(2k+2)\simeq \mathbb{C}l(2k)\otimes  End(\mathbb{C}^2)\simeq End(\mathbb{C}^{2^{k}}) \otimes End(\mathbb{C}^2)\simeq End(\mathbb{C^{2^k+1}})$$
		On the contrary, for $n$ odd, we still procede by induction and PROVAREEE
	\end{proof}
	\section{Real Clifford algebra's structure}
	In this chapter we briefly go through the analysis of the structure of real Clifford algebras. The idea is to use results REFERENZA and REFERENZA to find an immediate classification.
	\begin{Theo}
		The following decomposition holds:
		TABELLAAAA
	\end{Theo}
	\begin{proof}
		Recall that from REFERENZA, we have:
		$$\mathbb{C}l(n)\simeq Cl(n)\otimes \mathbb{C}$$
	\end{proof}
	\section{The spinor representation}
	\begin{Def}
		Let $Cl(V,Q)$ be a Clifford algebra. The \textit{Dirac spinor representation is}:
		$$\rho: Cl(n)\rightarrow End(\mathbb{C}^N)$$
		where this is defined accoarding to table REFERENZA.
		In particular:
		\begin{itemize}
			\item if $n$ is even:
			$$\rho:Cl(n)\xrightarrow{\simeq}End(\mathbb{C}^{2^{n/2}})$$
			\item if $n$ is odd, then:
			$$\rho:Cl(n)\xrightarrow{\simeq}End(\mathbb{C}^{2^{(n-1)/2}})\oplus End(\mathbb{C}^{2^{(n-1)/2}})\xrightarrow{Pr_1}End(\mathbb{C}^{2^{(n-1)/2}})$$
		\end{itemize}
	\end{Def}
	Note that, from REFERENZA, we have:
	$$\mathbb{C}l^{even}(n)=\mathbb{C}l(n-1)$$
	This automatically defines for us a spin representation for the even part of the Clifford algebra. In particular:
	\begin{itemize}
		\item if $n$ is odd:
		$$\rho:\mathbb{C}l^{even}(n)\xrightarrow{\simeq}End(\mathbb{C}^{2^{{n-1}\over 2}})$$
		This is irreducible;
		\item if $n$ is even:
		$$\rho:\mathbb{C}l^{even}(n)\xrightarrow{\simeq}End(\mathbb{C}^{2^{{n-2}\over 2}})\oplus End(\mathbb{C}^{2^{{n-2}\over 2}})$$
		We call this decomposition \textit{Weyl decomposoition}. In particular:
		$$\rho:\mathbb{C}l^{even}(n)\xrightarrow{\simeq}End(\Delta^+)\oplus End(\Delta^-)$$
		it decomposes into a \textit{right handed} and a \textit{left handed} spinor.
	\end{itemize}
	\section{Weyl and Majorana spinors}
	\section{The Spin group}
		\begin{Def}
		We dewfine the \textit{Pin group} $Pin(n)$ of a vector space as:
		$$Pin=\big\{ v_1\otimes...\otimes v_k\hbox{ with }Q(v_i)=\pm1\big\}$$
		We define the \textit{Spin group} of the vector space $V$ as 
		$$Spin(n)=Pin(V)\cap Cl^{even}(V,Q)$$
		$$Spin(n)=\big\{v_i\otimes...\otimes v_{2k}\hbox{ like }Q(v_i)=\pm1\big\}$$
		Lastly, the \textit{properly orthochronous spin group is}:
		$$Spin^+(n)=\big\{v_i\otimes...\otimes v_{2k}\hbox{ like }Q(v_i)=1\big\}$$
	\end{Def}
	...
	To see they are inverses, choose an orthonormal basis for the form (it always exists if the form is non degenerate). Then:
	...
	The $Spin$ and the $Pin$ group are clearly closed under this inversion. We endow those spaces with the subset topology from $Cl(V,Q)$, which is a vector space.\\
	\\
	The trouble now is the usual one: showing that those groups are indeed much well defined an that they inherit the structure of a Lie group.
	\begin{Theo}
		Consider the map:
		$$T:Pin(s,t)\rightarrow O(s,t)$$$$T(v_1\cdot...\cdot v_n)(x)=(-)^n v_1v_2...v_n x v_n^{-1}...v_1^{-1} $$
		The following holds:
		\begin{itemize}
			\item[(1)] This is a surjective homomorphism with $Kernel=\pm 1$ i.e. it is a double cover;
			\item[(2)] The pre-images of $SO(s,t)$ and $SO^+(s,t)$ are exactly $Spin$ and $Spin^+$;
			\item[(3)] $T$ restricts to surjective homomorphism on $Spin$ and $Spin^+$;
			\item[(4)] for $s \hbox{ or } t\geq 2$ $Spin$ and $Spin^+$ are connected.
		\end{itemize}
	\end{Theo}
	\begin{proof}
		The map is clearly a homomorphism since it is a multiplication row-columns. The vector $x$ is in $\mathbb{R}^n$ and so it can be canonically seen as an element of $Cl(n)$. This means that a generic element $I_Q$ acts as:
		$$v^2x+Q(v)x=v^2x+xQ(v)$$
		Moreover, from $\{v,w\}=2\braket{v,2}$, one immediatley sees:
		$$v^2x=xv^2$$
		This proves that:
		$$I_QxI_Q^{-1}=x$$
		This means that the ideal acts trivially as a rotation and proves the independence from the representative.\\
		\\
		The surjectivity of this map is follows from the fact that any matrix can be seen as a product of reflections with unit vectors (Cartan-Dieudonné theorem) REFERENZA.
		The kernel of this map can be found in the usual way:
		$$x\mapsto x\iff [v]=\pm[1]$$
		Now, it is clear that this is a double cover. If we look at the pre-images of the spaces $SO(s,t)$ and $SO(s,t)^+$, we have that the condition $det(A)=1$ can occurr only for an even number of conjugatuions, since the $-1$ eigenvalues can appear an even amount of times and every conjugation corresponds to a matrix. This proves that the pre-image of $SO(n)$ is $Spin$. In the same way, for $A\in SO^+(n)$, we must have $A^0_0\geq 1$. This means that, since the identity component of $Pin$ maps in the identity component of $O$, we must have that the pre-image of $SO^+$ is exactly $Spin^+$.
		\\\\
		Finally, to prove the connectedness, let $s\geq 2$ w.l.o.g. and take $v\in Spin^+(n)$. Consider the curve:
		$$\gamma(t)=-v(e_1\cos(\theta)+e_2\sin(\theta))(e_1\cos(\theta)-e_2\sin(\theta))$$
		For any $t\in \mathbb{R}$, this curve is in $Spin^+(n)$ since it is just a regular rotation in a plane applied 2 times. Notice that:
		$$\gamma(0)=-v\hbox{ while }\gamma(\pi/2)=v$$
		We have connected any element to its inverse. This proves that indeed the group is connected.
	\end{proof}
	\begin{Theo}
		The $Pin$, $Spin$ and $Spin^+$ group are Lie groups.
	\end{Theo}
	\begin{proof}
		Define $Cl(n)^\times$ the group of invertible elements of $Cl(n)$. This is an open subset of $Cl(n)$ since one can take any faithful matrix representation and this reduces to a determinant condition. Hence, $Cl(n)^\times$ is a Lie group.
		$Pin$ is a closed submanifold of $Cl^\times(n)$, so it is a closed subgroup. From REFERENZA, it is a Lie group. Now, from REFERENZA, since the map $T$ is clearly continuous open and surjective, we have that $Spin$ and $Spin^+$ are Lie groups.
	\end{proof}
	\begin{Obs}
		The above results allow us to define the $Spin^+$ representation by restriciting the one in $\mathbb{C}l^{even}(n)$ like:
		$$\rho:Spin^+(s,t)\rightarrow End(\mathbb{C}^N)$$
	\end{Obs}
	\section{The Spinor algebra}
	\section{Spin structures}
	\begin{Def}
		Let $M$ be a manifold with a pseudo-Riemannian metric $g$ of signature $(s,t)$. Let this be orientable so that the frame bundle can be reduced to an $SO^+(s,t)$ principal bundle: $Fr_{SO}(M)\xrightarrow{\pi_{SO}}M$. A \textit{Spin structure on } $M$ is a $Spin^+(s,t)$-principal bundle: $\mathcal{SP}\xrightarrow{\pi_{Spin^+}}M$ with a double covering map $\Lambda:\mathcal{SP}\rightarrow FR_{SO(n)}$ such that the following diagram commutes:	
		\begin{center}
			$$
			\begin{tikzcd}
				\mathcal{SP}\times Spin^+(s,t)\arrow{dddd}{\Lambda\times T}\arrow{rr}{Pr_2}&&Spin^+(s,t)\arrow{dddd}{T}\arrow{ddr}{\pi_{Spin^+}}&\\\\
				&& & M\\\\
				Fr_{SO}(M)\times SO^+(s,t)\arrow{rr}{Pr_2} &&SO^+(s,t)\arrow{uur}{\pi_{SO}}&\\\\
			\end{tikzcd}$$
		\end{center}
	\end{Def}
		\begin{Theo}
			The frame bundle $SO^+(M)$ admits a spin structure if and only if the second Stiefel–Whitney class of $M$ vanishes i.e  $w_2(M)=0$.
			\\
			If $SO^+(M)$ admits a spin structure, then there is a bijection between the set
			of isomorphism classes of spin structures on $M$ and the cohomology group $H_1(M,\mathbb{Z}_2)$
		\end{Theo}
		\begin{proof}
			PROVAAAA
		\end{proof}
		We now look at some results concerning spin structures.
		\begin{Prop}
			Let $M$ be a manifold with a spin structure. For any section $e:U\rightarrow Fr_{SO}(M)$ of the frame bundle defined on a contractible open set $U\subset M$ there are precisely two local sections $\varepsilon_\pm:U\rightarrow\mathcal{SP}$ such that:
			$$\Lambda\circ\varepsilon_\pm=e$$
		\end{Prop}
		\begin{proof}
			Since the section is nice (continuous and smooth), it's image is contractible. Thus $s(U)$ is diffeomorphic to $U$. Thus:
			$$\Lambda\big|_{\Lambda^{-1}(s(U))0}\rightarrow U$$
			is a double cover and so there are precisely two sections:
			$$s_\pm:s(U)\rightarrow\Lambda^{-1}(s(U))\hbox{ such that } \Lambda\circ s_\pm=I$$
			Define:
			$$\varepsilon_\pm=s_\pm\circ e$$
			This proves the claim.
		\end{proof}
		\begin{Def}
			Let $M$ be endowed with a spin structure. Let $\rho:Spin^+(s,t)\rightarrow End(\mathbb{C}^N)$ be the $Spin$ representation. We define the \textit{Dirac} spinor bundle as the associated bundle arising from the spin representation:
			$$\mathcal{SP}\times_\rho \mathbb{C}^N$$
		\end{Def}
		\begin{Def}
			A \textit{Dirac spinor} is a section of the Dirac spinor bundle.
		\end{Def}
		\section{Spin covariant derivative}
		In this section we dive into the construction of a covariant derivative on bundles wndowed with a spin structure. The idea is to induce a covariant derivative from the Levi-Civita connection, which is already canonically defined on the Frame bundle/Tangent bundle due to the metric.
		\begin{Obs}
			Recall that on a metric manifold $(M,g)$ there is a reduction of the frame bundle $Fr_{SO}(M)$ and a canonical Levi-Civita spin connection $A$ such that:
			$$A\in\Omega^1(Fr_{SO}(M),\mathfrak{so}^+(s,t))$$
			This was found in REFERENZA.
		\end{Obs}
		\begin{Obs}
			If $M$ is endowed with a spin structure, we have a double covering:
			$$T:Spin^+(s,t)\rightarrow SO^+(s,t)$$
			This descends immediately from REFERENZA, to a Lie algebra isomomorphism:
			$$t:\mathfrak{spin}^+(s,t)\xrightarrow{\simeq}\mathfrak{so}^+(s,t)$$
		\end{Obs}
		\begin{Prop}
			Let $M$ be a manifold endowed with a spin structure. Then there is an Ehresmann connection:
			$$A_{Spin}=t^{-1}\circ \Lambda^*A\in A\in\Omega^1(\mathcal{SP},\mathfrak{spin}^+(s,t))$$
		\end{Prop}
		\begin{proof}
			Clearly $A_{Spin}$ is smooth. Let $g\in Spin^+(s,t)$, then:
			$$r_g^*A_{Spin}=A_{Spin}\circ (r_g)_*=t^{-1}\circ \Lambda^*A\circ (r_g)_*$$
			Clearly, $\Lambda$
		\end{proof}
		\begin{Def}
			We call $A_{Spin}$ the \textit{Spin} connection.
		\end{Def}
	\chapter{Universal bundles}
	\section{Grassmannians}
	In this section we define the notion of Grassmannian and see some examples and some properties of it.\\
	\\
	We first give a more intuitive definition, in order to construct the rigorous one.
	\begin{Def}
		We call \textit{Grassmannian} $G_n(\mathbb{R}^{n+k})$ the set of all $n$-dimensional planes of $\mathbb{R}^{n+k}$ passing through $\vec{0}$.
	\end{Def}
	This is very intuitive, not much formal. 
	\begin{Def}
		We call \textit{Stiefel manifold } the set $V_n(\mathbb{R}^{n+k})$ containing all $n$-tuples of linearly independent vectors of $\mathbb{R}^{n+k}$.
	\end{Def}
	\begin{Obs}
		Clearly:
		$$V_n(\mathbb{R}^{n+k})=\big\{A\in M(\mathbb{R})_{n+k,n}\hbox{ such that } rk(A)=n\big\}$$
		Since the rank of such matrices is full if at least one of its $n\times n$ sub-matrices has non 0 determinant, this makes $V_k(\mathbb{R}^{n+k})$ an open subset of $M(\mathbb{R})_{n+k,n}$ and, from REFERENZA, also a smooth manifold of dimension $n(n+k)$.
	\end{Obs}
	\begin{Obs}
		There is a canonical mapping:
		$$q:V_n(\mathbb{R}^{n+k})\longrightarrow G_n(\mathbb{R}^{n+k})$$
		This is defined as sending each $n$-tuple to its plane through the origin. Oone can use this map to induce a topology on $G_n(\mathbb{R}^{n+k})$. This is what we will do.
	\end{Obs}
	\begin{Theo}
		$G_n(\mathbb{R}^{n+k})$ is a smooth manifold.
	\end{Theo}
	\begin{proof}
		The hardest part is showing that this set inherits the nice topologival properties of a topologival manifold. PROVA
	\end{proof}
	\chapter{Jets}
